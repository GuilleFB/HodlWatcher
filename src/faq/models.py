...
class FAQ(models.Model):
    ...
    category = models.ForeignKey(
        FAQCategory, on_delete=models.CASCADE, related_name="faqs", verbose_name=_("Categoría"), null=True, blank=True
    )
    order = models.PositiveIntegerField(default=0, verbose_name=_("Orden"))
    slug = models.SlugField(max_length=255, unique=True, blank=True)
    is_active = models.BooleanField(default=True, verbose_name=_("Activo"))
    related_questions = models.ManyToManyField(
        "self", blank=True, symmetrical=False, verbose_name=_("Preguntas relacionadas"), related_name="related_to"
    )
